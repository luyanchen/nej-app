...
NEJ.define([
    ...
    //ie7-浏览器先执行初始行为-by cathy
    _p._$init = (function(){
        var inited = !1; //初始化标识-by cathy
        return function(){
            if (inited){
                return;
            }
            ...
            var __doFlushTempData = (function(){
                var _doFlush = function(_value,_key,_map){
                    _h.__setItemToStorage(
                        _key,JSON.stringify(_value));  //storage键值对-by cathy
                    delete _map[_key];
                };
                return function(){
                    _u._$loop(_cache,_doFlush); //遍历清空对象_cache
                };
            })();
            _v._$addEvent(
    ...
    _p._$setDataInStorage = function(_key,_value){
        ...
        // set failed 本地存储失败，存放在_cache对象中-by cathy
